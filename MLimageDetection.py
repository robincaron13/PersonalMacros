...
#    Please download the SSD ONNX model (ssd.onnx) file from here
#    Please download the ONNXRuntime Server image:
#  4) sudo docker pull mcr.microsoft.com/onnxruntime/server
#    In the same folder as the downloaded ssd.onnx file, please run (to be run on Linux, for Windows you must use WSL):
#
#  5) sudo docker run -it -v $(pwd):$(pwd) -p 9001:8001 mcr.microsoft.com/onnxruntime/server --model_path $(pwd)/ssd.onnx
#
